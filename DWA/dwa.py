...
class Obstacles():
    ...
    def assignObs(self, msg, config):
        ...
        print('The range of the laser %s',deg )

        self.obst = set()   # reset the obstacle set to only keep visible objects
        for angle in self.myRange(0,deg-1,16): #deg-1 (0,deg-1,16)
            ...
            if (distance < 2.0):
                ...
                obsX = round((config.x + (distance * math.cos(abs(objTheta))))*8)/8 #/8
...
def calc_dynamic_window(x, config):
    ...
    Vs = [config.min_speed, config.max_speed,
          -config.max_yawrate, config.max_yawrate +0.1]
...
def calc_obstacle_cost(traj, ob, config):
    ...
    for ii in range(0, len(traj[:, 1]), skip_n):
        for i in ob.copy():
            ...
            if r <= config.robot_radius+ 0.05: #burayi degistirdin 0.05 yokttu
...
def atGoal(config, x):
    ...
    if math.sqrt((x[0] - config.goalX)**2 + (x[1] - config.goalY)**2) \
        <= config.robot_radius+ 0.15 :
        return True
    return False


def main():
    print( "Dynamic Window Approach started !!")
    # robot specification
    config = Config()
    ...
    while not rospy.is_shutdown():
        if (atGoal(config,x) == False):
            ...
            speed.linear.x = x[3]
            speed.angular.z = x[4]
            print('The Linear Speed is :',speed.linear.x , '->> The angular Speed is :',speed.angular.z)
